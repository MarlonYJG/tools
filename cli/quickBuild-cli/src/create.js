...
const { updateJsonFile, templateCopy, copyDir, downloadHubApp, isExistGst, downloadTemplate, createWorkSpace, log, prompt, creatPro } = require('../lib/utils')
...
exports.createWrokspace = async function (...args) {
    let repositoriesMap = {
        '': {
            name: '',
            github: 'https://xxx.git'
        }
    }
    let workespaceName = args[1];
    let wk = await createWorkSpace(workespaceName);
    // 进入.gst文件并下载库
    await downloadHubApp('组件库(dataplatform)', 'https://xxx.git', `${process.cwd()}/${workespaceName}/.gst`, 'master')
    if (wk) {
        let repo_choices = [];
