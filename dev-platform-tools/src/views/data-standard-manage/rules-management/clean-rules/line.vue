        ...
        .then((data) => {
          if (data.data.code === "0") {
            ...
            if (Object.keys(this.DigRFormInit).length) {
                  ...
                  if (initObj.hasOwnProperty(key)) {
                    if (item.prop === key) {
                      if (item.displayMode === 6) {
                        if (item.paraType === 4) {
                          // 下拉+url
                          console.log("下拉=url", item, initObj, key);
              ...
              this.DigRForm = Res;
              console.log("编辑-初始值", this.DigRForm);
            } else {
              // 新增
              Res.forEach((item) => {
                // 默认值的格式初始化
