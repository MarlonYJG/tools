...
export default {
          ...
          cb: () => {
            ...
            this.GTgraphData.nodes.forEach((item) => {
              if (this.activeNode_.id == item.id) {
                this.$set(item, `arrangeTableMappingId`, this.value);
                this.$set(item, `tableMappingName`, name_);
              }
            });
            ...
            console.log(this.activeNode_, "当前节点", "转换表");
            if (this.activeNode_.state == 0 || this.activeNode_.state == 2) {
                ...
                if (this.activeNode_.id == item.target) {
                  this.GTgraphData.nodes.forEach((item_) => {
                    if (item.source == item_.id) {
                      this.reqData.push({
                        arrangeTableMappingId: item_.arrangeTableMappingId,
                        childArrangeTableMappingId: this.value,
                        childNodeNum: item_.childNodeNum,
                        nodeNum: item_.id,
                        state: 2,
                        id: item_.node_id,
                        tableMappingName: item_.tableMappingName,
                        xAxis: item_.x,
                        yAxis: item_.y,
                        taskOrTable: "table",
                        addType: item_.addType || "",
                      });
                    }
                  });
                }
              ...
              // 修改当前节点(返回数据)
              this.reqData.push({
                arrangeTableMappingId: this.value,
                childArrangeTableMappingId: this.activeNode_
                  .childArrangeTableMappingId,
                childNodeNum: this.activeNode_.childNodeNum,
                nodeNum: this.activeNode_.id,
                state: 2,
                id: this.activeNode_.node_id,
                tableMappingName: this.activeNode_.tableMappingName,
                xAxis: this.activeNode_.x,
                yAxis: this.activeNode_.y,
                taskOrTable: "table",
                addType: this.activeNode_.addType || "",
              });
            } else {
              // 获取当前节点作为前置节点所关联的所有后置(子)节点——前置
              let actNode = {}; // 当前
              this.reqData.forEach((item) => {
                if (this.activeNode_.id == item.nodeNum) {
                  console.log(item, "当前点击的值");
                  actNode = item;
                  actNode[
                    "tableMappingName"
                  ] = this.activeNode_.tableMappingName;
                  actNode["arrangeTableMappingId"] = this.value;
                }
              });

              // 在生成的数据中查找并修改
              this.reqData.forEach((item) => {
                console.log(actNode, this.activeNode_);
                // 修改当前节点（前置）
                if (actNode.childNodeNum == item.nodeNum) {
                  actNode.childArrangeTableMappingId =
                    item.arrangeTableMappingId;
                  actNode.arrangeTableMappingId = this.value;
                }
              });
              ...
              this.reqData.forEach((item) => {
                console.log(actNode, this.activeNode_);
                // 修改当前节点（后置）
                if (actNode.nodeNum == item.childNodeNum) {
                  item.childArrangeTableMappingId = this.value;
                  actNode.arrangeTableMappingId = this.value;
                }
              });
            }
        ...
        if (newVal && this.GTreeActive) {
          ...
          if (this.GTreeActive) {
            if (this.GTreeActive.info.flag == "task") {
              // 任务
              taskName_ = this.GTreeActive.label;
              if (this.GTreeActive.children.length) {
                activeTreeTaskId = this.GTreeActive.children[0].id;
              }
            } else if (this.GTreeActive.info.flag == "table") {
              taskName_ = this.GTreeActive.info.parentName;
              // 表
            ...
            if (newVal.nodes.length) {
              ...
              this.GTgraphData = {
                nodes: [
                  {
                    id: nodeNumIndex,
                    childArrangeTableMappingId: 0,
                    childNodeNum: 0,
                    taskName: taskName_,
                    // arrangeTaskId: "",
                    nodeNum: nodeNumIndex,
                    taskOrTable: "table",
                    tableMappingName: activeTreeTaskName,
                    arrangeTableMappingId: activeTreeTaskId
  ...
  methods: {
    ...
    initEvent() {
      graph.on("node:click", async (evt) => {
          ...
          case "前置":
            {
              ...
              // 存储生成的父节点
              this.reqData.push({
                arrangeTableMappingId: "",
                childArrangeTableMappingId:
                  model.childArrangeTableMappingId || 0,
                childNodeNum: model.id,
                nodeNum: modelId,
                state: 1,
                tableMappingName: "",
          ...
          case "后置":
            {
              ...
              if (!Number(nowNode.childNodeNum)) {
                ...
                if (model.state == 0) {
                  this.reqData.push({
                    arrangeTableMappingId: model.arrangeTableMappingId,
                    childArrangeTableMappingId: 0,
                    ...
                    id: model.node_id,
                    taskName: this.taskName_,
                    tableMappingName: model.tableMappingName,
                    xAxis: model.x,
                    yAxis: model.y,
                    taskOrTable: "table",
                    addType: "h",
        ...
        if (dialog || (width && height)) {
          console.log("弹出框-选择");
          this.value = model.arrangeTableMappingId;
      ...
      G6.registerNode(
        "flow-rect",
        {
          ...
          draw(cfg, group) {
            ...
            // 标题判断
            function ItemTitle() {
              console.log($this.GraphTDigTitle, cfg, "=====转换绘制节点=====");
              let title = "";
              if (cfg.taskOrTable == "table") {
                title = "任务：" + $this.taskName_;
                if (title.length > 10) {
                  title = title.substr(0, 10) + "...";
                } else {
                  title = title;
                }
                let tableMappingName_ = "转换：" + tableMappingName;
                if (tableMappingName_.length > 10) {
                  title =
                    title +
                    "\n" +
                    "转换：" +
                    tableMappingName.substr(0, 10) +
                    "...";
                } else {
                  title = title + "\n" + "转换：" + tableMappingName;
                }
              } else if (cfg.taskOrTable == "task") {
                title = "任务：" + cfg.taskName;
                if (title.length > 10) {
                  title = title.substr(0, 10) + "...";
                } else {
                  title = title;
    ...
    //
    initGraph(data) {
      const width = document.getElementById(`g2`).scrollWidth;
      const height = document.getElementById(`g2`).scrollHeight || 500;
      const defaultConfig = {
        width,
        height,
        pixelRatio: 1,
      ...
      const { config } = props;
      graph = new G6.Graph(
        Object.assign(Object.assign({ container: `g2` }, defaultConfig), config)
      );
      // 初始化数据
      this.initData(data);
      // 注册事件
